...
class pascalVOCLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augmentations is not None:
            opt = self.augmentations(image=im, mask=lbl)
            im, lbl = opt["image"], opt["mask"]
    ...
    def setup_annotations(self):
        ...
        if len(pre_encoded) != expected:
            ...
            for ii in tqdm(sbd_train_list):
                ...
                nparr = np.asarray(lbl, dtype=np.uint8)
                if nparr.max() == 0:
                    print("Warning: empty segmentation mask")
                Image.fromarray(nparr).save(pjoin(target_path, ii + ".png"))
                #lbl = m.toimage(lbl, high=lbl.max(), low=lbl.min())
                #m.imsave(pjoin(target_path, ii + ".png"), lbl)
            
            #for ii in self.files["trainval"]:
            for ii in tqdm(self.files["trainval"]):
                fname = ii + ".png"
                lbl_path = pjoin(self.root, "SegmentationClass", fname)
                mask = np.array(Image.open(lbl_path).convert('RGB'), dtype=np.uint8)
                lbl = self.encode_segmap(mask)
                Image.fromarray(np.asarray(lbl, dtype=np.uint8)).save(pjoin(target_path, fname))
                #lbl = m.toimage(lbl, high=lbl.max(), low=lbl.min())
...
if __name__ == "__main__":
    ...
    pascal_loader = pascalVOCLoader(voc_path, benchmark)
