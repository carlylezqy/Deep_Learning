...
Regularization 或称 Weight Decay(权重衰减)
...
权重更新的方向为当前梯度值 $\nabla f\left(w^{k}\right)$ 以及上一次的梯度方向 $z_{k}$
...
net.eval() 时 dropout() 不被激活执行

+ Stochastic Gradient Descent

https://ruder.io/optimizing-gradient-descent/

not random! [Stochastic/Deterministic]
...
Intuitive explanation(直观解释)：为了使梯度下降时，使loss函数的等高线均匀分布，最优解的寻优过程明显会变得平缓，使其更快收敛。
...
Every Layer is nn.Module. eg. ```nn.Linear```, ```nn.BatchNorm2d```, ```nn.Conv2d```

nn.Module nested in nn.Module. Module可以互相嵌套.
...
Limited Data: (1)Small Network Capacity; (2)Regularization; (3)Data argumentation;
