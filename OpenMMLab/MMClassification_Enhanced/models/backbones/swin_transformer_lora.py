...
from ..utils import (ShiftWindowMSALoRA, resize_pos_embed,
...
class SwinTransformerLoRA(BaseBackbone):
    """Swin Transformer.
    ...
    def __init__(self,
                 ...
                 patch_cfg=dict(),
                 init_cfg=None):
        super(SwinTransformerLoRA, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(SwinTransformerLoRA, self).init_weights()
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, *args,
        ...
        if (version is None
                or version < 2) and self.__class__ is SwinTransformerLoRA:
            ...
            for k in state_dict_keys:
                if k.startswith('norm.') or k.startswith('backbone.norm.'):
                    ...
                    del state_dict[k]
        if (version is None
                or version < 3) and self.__class__ is SwinTransformerLoRA:
            state_dict_keys = list(state_dict.keys())
            for k in state_dict_keys:
                if 'attn_mask' in k:
                    del state_dict[k]
    ...
    def train(self, mode=True):
        super(SwinTransformerLoRA, self).train(mode)
        self._freeze_stages()
